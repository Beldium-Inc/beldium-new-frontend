...
function ApplicationPage() {
  ...
  useEffect(() => {
    if (organisationName && !app.org.legalName) {
      saveApplication((d) => ({ ...d, org: { ...d.org, legalName: organisationName } }));
    }
    // Pre-fill once, from the name captured at signup.
    ...
    try {
      ...
      for (const site of app.sites) {
        const created = await createMineSite({
          ...
          state: site.region,
          area_ha: site.hectares ? Number(site.hectares) : undefined,
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-8 px-6 py-10 lg:grid-cols-[260px_1fr]">
        ...
        <main className="rounded-md border border-border bg-card p-6" style={{ boxShadow: "var(--shadow-panel)" }}>
          ...
          <div className="mt-8 flex items-center justify-between border-t border-border pt-5">
            <Button variant="outline" disabled={step === 1 || submitting} onClick={() => goto(step - 1)}>
              Back
            </Button>
            <Button onClick={next} disabled={submitting}>
              {submitting ? "Submitting…" : step === 8 ? "Submit application" : "Save & continue"}
            </Button>
          </div>
        </main>
      </div>
