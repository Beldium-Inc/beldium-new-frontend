...
import { expiringDocuments } from "@/lib/mock-data";
...
function ExpiringDocs() {
  ...
  return (
    <AppShell
      role="operator"
      breadcrumbs={[{ label: "Compliance Operations", to: "/operator" }, { label: "Expiring documents" }]}
              ...
              {rows.map((d) => (
                <tr key={d.id} className="hover:bg-muted/40">
                  ...
                  <td className="px-3 py-3 text-xs">
                    <Link to="/operator/applications/$companyId" params={{ companyId: d.companyId }} search={{ tab: "overview" as const }} className="text-[var(--link)] hover:underline">
                      {d.company}
                    </Link>
                  </td>
