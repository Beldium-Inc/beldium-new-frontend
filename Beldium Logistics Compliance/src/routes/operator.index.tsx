...
import { KeyValue, PageHeader, Panel, Pill, RiskBadge, StatCard, StatusBadge } from "@/components/beldium/bits";
import { useApp } from "@/lib/app-state";
import { complianceTrend, expiringDocuments, riskDistribution } from "@/lib/mock-data";
  ...
  return (
    ...
    >
      <PageHeader
        ...
        actions={
          <>
            ...
            <Link
              to="/operator/applications/$companyId"
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 xl:grid-cols-4">
        <StatCard label="Applications" value={counts.applications} hint="Total in register" icon={Inbox} tone="info" onClick={() => navigate({ to: "/operator/applications" })} />
        <StatCard label="Pending reviews" value={counts.pending} hint="Awaiting operator action" icon={ClipboardList} tone="neutral" onClick={() => navigate({ to: "/operator/applications" })} />
        <StatCard label="Additional information" value={counts.info} hint="Open requests with partners" icon={MessageSquareWarning} tone="warning" onClick={() => navigate({ to: "/operator/requests" })} />
        <StatCard label="Expiring documents" value={counts.expiring} hint="Within 90 days" icon={FileClock} tone="warning" onClick={() => navigate({ to: "/operator/documents" })} />
        <StatCard label="High risk" value={counts.high} hint="Score below 60" icon={AlertOctagon} tone="danger" onClick={() => navigate({ to: "/operator/risk" })} />
        <StatCard label="Approved" value={counts.approved} hint="Incl. conditional approvals" icon={CheckCircle2} tone="success" onClick={() => navigate({ to: "/operator/applications" })} />
        <StatCard label="Rejected" value={counts.rejected} hint="Last 12 months" icon={XCircle} tone="danger" onClick={() => navigate({ to: "/operator/applications" })} />
        <StatCard label="Unread alerts" value={notifications.filter((n) => n.audience.includes("operator") && !n.read).length} hint="Platform notifications" icon={ShieldAlert} tone="info" onClick={() => navigate({ to: "/operator/notifications" })} />
      </div>
      ...
      <div className="mt-6 grid gap-4 xl:grid-cols-3">
        ...
        <Panel title="Critical expiries" description="Automatic monitoring across the register" bodyClassName="p-0">
          ...
          <div className="border-t border-border px-5 py-3">
            <Link to="/operator/documents" className="inline-flex items-center gap-1 text-xs font-medium text-[var(--link)] hover:underline">
