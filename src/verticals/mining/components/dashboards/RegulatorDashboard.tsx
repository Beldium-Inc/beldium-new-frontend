...
export function RegulatorDashboard() {
  ...
  const act = async (
    ...
    setBusy(siteId + kind);
    try {
      await regulatorAction(kind, siteId);
      toast.success(`${kind} recorded`, {
        description: `${siteId}: oversight action logged to the audit history.`,
      });
    } catch (err) {
  ...
  return (
    <>
      ...
      <div className="mb-5">
        <DemoNote>
          Oversight role is read-mostly: no verification, rejection or scoring changes are available
          here.
        </DemoNote>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          hint={`${organisations.filter((o) => o.status !== "Active").length} not in good standing`}
        />
        <KpiCard
          label="Mine sites"
          value={sites.length}
          hint={`${sites.filter((s) => s.status === "Operational").length} operational`}
        />
        <KpiCard
          label="Licences expiring or expired"
          value={licences.filter((l) => l.status !== "Active").length}
          tone="warning"
          hint="Renewal exposure across the register"
        />
        <KpiCard
          label="Open non-conformities"
          value={nonConformities.filter((n) => n.status !== "Closed").length}
          tone="danger"
          hint={`${nonConformities.filter((n) => n.severity === "Critical" && n.status !== "Closed").length} critical`}
        />
      </div>
      ...
      <div className="mt-5 grid gap-5">
        <Panel
          ...
          description="Licence and compliance standing by organisation."
          bodyClassName="p-0"
        >
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead className="text-right">Oversight action</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
                {organisations.map((o) => (
                  <TableRow key={o.id}>
                    <TableCell className="font-medium">
                      {o.name}
                      <p className="text-xs text-muted-foreground">{o.hq}</p>
                    </TableCell>
                    ...
                    <TableCell className="text-right">
                      <div className="flex flex-wrap justify-end gap-1.5">
                        <Button
                          ...
                          variant="outline"
                          disabled={busy !== null}
                          onClick={() => act("Send Reminder", o.id)}
                        >
                          Send reminder
                        </Button>
                        <Button
                          ...
                          variant="outline"
                          disabled={busy !== null}
                          onClick={() => act("Request Information", o.id)}
                        >
                          Request information
                        </Button>
                      </div>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>Licence status</TableHead>
                  <TableHead>Score</TableHead>
                  <TableHead>Risk</TableHead>
              ...
              <TableBody>
                {sites.map((s) => {
                  const lic = licences.find((l) => l.siteId === s.id);
                  return (
                    <TableRow key={s.id}>
                      <TableCell className="font-medium">
                        {s.name}
                      ...
                      <TableCell className="text-right">
                        <div className="flex flex-wrap justify-end gap-1.5">
                          ...
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => act("Acknowledge Submission", s.id)}
                          >
                            Acknowledge
                          </Button>
                        </div>
                ...
                {envRecords.map((e) => (
                  <TableRow key={e.id}>
                    ...
                    <TableCell className="text-sm">
                      {e.metric}
                      <p className="text-xs text-muted-foreground">Limit {e.limit}</p>
                    </TableCell>
