...
export function MinerDashboard() {
  const { sites, nonConformities, infoRequests, inspections, organisations } = useStore();
  // Reads are already scoped to the caller's own organisation, so "my sites"
  // is simply the whole `sites` list here; the first is treated as primary.
  const mine = sites;
  const site = mine[0];
  const org = organisations.find((o) => o.id === site?.orgId) ?? organisations[0];

  if (!site || !org) {
    return <DemoNote>No mine site is associated with this account yet.</DemoNote>;
  }
  const openNc = nonConformities.filter(
    (n) => mine.some((m) => m.id === n.siteId) && n.status !== "Closed",
  );
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <Button asChild>
            <Link to="/mining/actions">Open action required ({openNc.length + openIr.length})</Link>
          </Button>
        }
      />
      <div className="mb-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <KpiCard
          ...
          hint={`${org.risk} risk band`}
          tone={org.complianceScore >= 80 ? "success" : "warning"}
        />
        <KpiCard
          label="Mine status"
          value={site.status}
          hint={`${site.code} · ${site.mineral}`}
          tone={site.status === "Operational" ? "success" : "warning"}
        />
        <KpiCard
          label="Licensed capacity"
          value={`${site.capacityTpa.toLocaleString()} t/yr`}
          hint={`Current run rate ${site.currentTpa.toLocaleString()} t/yr (${Math.round((site.currentTpa / site.capacityTpa) * 100)}%)`}
        />
        <KpiCard
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <Panel
          title={`Site dashboard: ${site.name}`}
          description="Verification status and production position."
        >
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            ...
            <Field label="Site verification">
              <StatusChip value={site.verification.site ? "Verified" : "Pending"} />
            </Field>
            <Field label="Licence verification">
              <StatusChip value={site.verification.licence ? "Verified" : "Pending"} />
            </Field>
            <Field label="Documents verification">
              <StatusChip value={site.verification.documents ? "Verified" : "Pending"} />
            </Field>
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <Button asChild size="sm" variant="outline">
              <Link to="/mining/production">Production</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link to="/mining/inventory">Inventory</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link to="/mining/transactions">Transactions</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link to="/mining/documents">Licences &amp; documents</Link>
            </Button>
            <Button asChild size="sm" variant="outline">
              <Link to="/mining/compliance">Compliance</Link>
            </Button>
          </div>
        </Panel>
            ...
            {openNc.map((n) => (
              <li key={n.id} className="px-5 py-3.5">
                ...
                <Button asChild size="sm" variant="outline" className="mt-2">
                  <Link to="/mining/actions">Respond</Link>
                </Button>
              </li>
            ))}
            {openIr.map((r) => (
      ...
      <div className="mt-5 grid gap-5 lg:grid-cols-2">
        ...
        <Panel title="Upcoming & recent inspections" bodyClassName="p-0">
          <Table>
            ...
            <TableBody>
              {inspections
                .filter((i) => mine.some((m) => m.id === i.siteId))
                .map((i) => (
                  <TableRow key={i.id}>
