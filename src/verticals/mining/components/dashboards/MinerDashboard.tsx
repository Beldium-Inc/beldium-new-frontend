...
import { MINER_ORG_ID, MINER_PRIMARY_SITE, organisations } from "@/verticals/mining/data";
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)]">
        <Panel title={`Site dashboard: ${site.name}`} description="Verification status and production position.">
          ...
          <div className="mt-5 flex flex-wrap gap-2">
            <Button asChild size="sm" variant="outline"><Link to="/mining/production">Production</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/mining/inventory">Inventory</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/mining/transactions">Transactions</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/mining/documents">Licences &amp; documents</Link></Button>
            <Button asChild size="sm" variant="outline"><Link to="/mining/compliance">Compliance</Link></Button>
          </div>
        </Panel>
          ...
          <ul className="divide-y divide-border">
            {openNc.map((n) => (
              <li key={n.id} className="px-5 py-3.5">
                ...
                <Button asChild size="sm" variant="outline" className="mt-2"><Link to="/mining/actions">Respond</Link></Button>
              </li>
            ))}
            {openIr.map((r) => (
            ...
            <TableBody>
              {mine.map((s) => (
                <TableRow key={s.id}>
                  ...
                  <TableCell className="text-right"><Button asChild size="sm" variant="outline"><Link to="/mining/sites/$siteId" params={{ siteId: s.id }}>View</Link></Button></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
