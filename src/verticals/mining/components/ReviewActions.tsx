...
const meta: Record<
  Exclude<SectionDecision, "Request Information">,
  { title: string; prompt: string; cta: string }
> = {
  Verify: {
    title: "Verify section",
    prompt: "Confirm the evidence supports compliance. Add a reviewer note for the audit trail.",
    cta: "Record verification",
  },
  Reject: {
    title: "Reject section",
    prompt: "State the deficiency. The organisation will see this reason.",
    cta: "Record rejection",
  },
  "Request Inspection": {
    title: "Request inspection",
    prompt: "Describe what the field inspector should verify on site.",
    cta: "Raise inspection request",
  },
  Flag: {
...
export function ReviewActions({
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await decideSection(siteId, section, open, note.trim());
      toast.success(`${open} recorded`, {
        description: `${section} section updated on ${siteId}.`,
      });
      setNote("");
      setOpen(null);
    } catch (err) {
            ...
            <>
              ...
              <DialogFooter>
                ...
                <Button onClick={() => void submit()} disabled={submitting}>
                  {submitting ? "Saving…" : meta[open].cta}
                </Button>
              </DialogFooter>
            </>
          )}
