...
const STORAGE_KEY = "beldium.logistics.v1";
...
export function AppStateProvider({
  ...
  const seat = React.useMemo<Session | null>(() => {
  ...
  const [store, setStore] = React.useState<Store>({ ...initialStore, session: seat });
  const [hydrated, setHydrated] = React.useState(false);

  React.useEffect(() => {
    try {
      const raw = window.localStorage.getItem(STORAGE_KEY);
      if (raw) setStore((s) => ({ ...initialStore, ...(JSON.parse(raw) as Store), session: s.session }));
    } catch {
      /* ignore */
    }
    setHydrated(true);
