...
export function Shell({ children }: { children: ReactNode }) {
  const { user: account, orgName, logout, notifications, markNotificationsRead } = useMiner();
  ...
  const [collapsed, setCollapsed] = useState(() => {
    ...
    try {
      return window.localStorage.getItem("miner-nav-collapsed") === "1";
    } catch {
      return false;
    }
  });
  ...
  useEffect(() => {
    try {
      window.localStorage.setItem("miner-nav-collapsed", collapsed ? "1" : "0");
    } catch {
              ...
              <ul className="space-y-0.5">
                {items
                  ...
                  .map((item) => {
                    ...
                    const active =
                      pathname === item.to ||
                      (item.to !== "/miner/dashboard" && pathname.startsWith(item.to));
                    return (
                      <li key={item.to}>
                        <Link
                          to={item.to}
