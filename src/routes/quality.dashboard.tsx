...
import { EmptyState, Field, PageHeader, Pill, SectionTitle, Stat, StatusPill, Surface } from "@/verticals/quality/ui";
import { useBeldium } from "@/verticals/quality/store";
import { roleMeta } from "@/verticals/quality/data";

export const Route = createFileRoute("/quality/dashboard")({
  head: () => ({
    meta: [
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-[1.4fr_1fr]">
            <Surface>
              <SectionTitle
                ...
                action={
                  <Link to="/quality/applications" className="text-sm font-medium text-link hover:underline">
                    Open queue
                  </Link>
                }
              />
      ...
      {role === "partner" ? (
        <>
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-2">
            <Surface>
              <SectionTitle title="Bench workload" action={<Link to="/quality/samples" className="text-sm font-medium text-link hover:underline">All samples</Link>} />
              <div className="divide-y divide-border">
                {samples
                  .filter((s) => s.status !== "certified")
                  .map((s) => (
                    <Link key={s.id} to="/quality/samples/$id" params={{ id: s.id }} className="flex items-center justify-between gap-3 px-6 py-4 hover:bg-accent/50">
                      <div>
      ...
      {role === "miner" ? (
        <>
          ...
          <Surface className="mt-6">
            <SectionTitle title="My lots" hint="Register a sample and follow it to certification" action={<Link to="/quality/samples" className="text-sm font-medium text-link hover:underline">Register sample</Link>} />
            <div className="divide-y divide-border">
              {samples.map((s) => (
                <Link key={s.id} to="/quality/samples/$id" params={{ id: s.id }} className="flex flex-wrap items-center justify-between gap-3 px-6 py-4 hover:bg-accent/50">
                  <div>
        ...
        <>
          ...
          <div className="mt-6 grid gap-6 xl:grid-cols-2">
            <Surface>
              ...
              <div className="divide-y divide-border">
                {applications.map((a) => (
                  <Link key={a.id} to="/quality/applications/$id" params={{ id: a.id }} className="flex items-center justify-between gap-3 px-6 py-4 hover:bg-accent/50">
                    <div>
