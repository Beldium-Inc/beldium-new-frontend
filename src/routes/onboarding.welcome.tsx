...
function WelcomePage() {
  ...
  const enter = () => {
    ...
    // back to mining only if someone reaches this page with no sector recorded.
    const slug = sector ?? "mining";
    const seat = seatFor(slug, role);
    markWelcomeSeen();
    signIn(slug, seat);
    navigate({ to: homeFor(slug, seat) });
  };
