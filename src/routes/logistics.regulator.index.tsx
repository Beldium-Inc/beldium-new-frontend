...
import { PageHeader, Panel, Pill, ReadOnlyNotice, StatCard, StatusBadge } from "@/verticals/logistics/bits";
import { useApp } from "@/verticals/logistics/store";

export const Route = createFileRoute("/logistics/regulator/")({
  head: () => ({
    meta: [
            ...
            {companies.slice(0, 8).map((c) => (
              <li key={c.id} className="flex flex-wrap items-center gap-3 px-5 py-3.5">
                <div className="min-w-0 flex-1">
                  <Link to="/logistics/regulator/operators" className="text-sm font-medium text-[var(--brand)] hover:text-[var(--link)]">
                    {c.name}
                  </Link>
