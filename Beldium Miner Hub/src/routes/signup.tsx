...
import type { MinerRole } from "@/lib/miner-data";
...
const roles: { id: MinerRole; icon: typeof Building2; title: string; body: string }[] = [
  {
    id: "org_admin",
    icon: Building2,
...
function SignupPage() {
  const { registerAccount, update } = useMiner();
  const navigate = useNavigate();
  const [role, setRole] = useState<MinerRole>("org_admin");
  const [form, setForm] = useState({ fullName: "", email: "", phone: "", organisationName: "", password: "" });
  ...
  function submit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.fullName || !form.email.includes("@") || form.phone.length < 6 || form.password.length < 6) {
      setError("Complete your name, a valid email, a phone number and a 6+ character password.");
      return;
    }
    ...
    if (role === "org_staff") {
      update((d) => {
        d.joinRequest = {
          organisationName: form.organisationName,
          reference: `JR-${Math.floor(100000 + Math.random() * 899999)}`,
          status: "pending",
