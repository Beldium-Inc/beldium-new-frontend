...
import { statusLabel } from "@/lib/miner-data";
...
export function MinerShell({ children }: { children: React.ReactNode }) {
  const { state, signOut, setOrgStatus } = useMiner();
  const navigate = useNavigate();
  const pathname = useRouterState({ select: (s) => s.location.pathname });
  const verified = state.orgStatus === "verified";
          ...
          <header className="flex h-16 shrink-0 items-center gap-3 border-b border-border bg-card px-4">
            ...
            <div className="min-w-0">
              <div className="truncate text-sm font-medium text-foreground">
                {state.account?.organisationName || state.account?.fullName || "Miner workspace"}
              </div>
              <div className="text-xs text-muted-foreground">{state.account?.email}</div>
            </div>
            <div className="ml-auto flex items-center gap-2">
              <StatusChip tone={verified ? "success" : "warning"}>{statusLabel[state.orgStatus]}</StatusChip>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setOrgStatus(verified ? "under_review" : "verified")}
              >
                {verified ? "Demo: back to review" : "Demo: approve verification"}
              </Button>
              <Button
                variant="ghost"
                size="sm"
