...
const SESSION_KEY = "beldium.demo.session";
...
export function DemoProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = window.localStorage.getItem(SESSION_KEY) as RoleId | null;
    if (stored === "partner" || stored === "operator" || stored === "regulator") setRoleId(stored);
  }, []);
  ...
  const signIn = useCallback((role: RoleId) => {
