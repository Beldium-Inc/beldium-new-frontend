...
const STORAGE_KEY = "beldium-demo-state-v1";
...
export function StoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const actorName = state.user?.name ?? "Demo user";
  const actorRole: Role | "system" = state.user?.role ?? "system";
  ...
  const value: Ctx = {
    ...
    resetDemo: () => setState({ ...initialState, user: state.user }),
    ...
    addShipment: (draft) => {
      ...
      const exporterId = state.user?.exporterId ?? "EXP-1042";
