...
export const Route = createFileRoute("/_ops/drivers_/$driverId")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.driverId} | Driver | Beldium Logistics` },
      { name: "description", content: `Driver ${params.driverId} profile: licence, documents, training, assignment, history and incidents.` },
      { property: "og:title", content: `${params.driverId} | Beldium Logistics` },
...
function DriverProfile() {
  ...
  if (!d) {
    return (
      <Panel title="Driver not found">
        <Link to="/drivers" className="text-sm font-semibold text-colorLink">
          Back to Drivers
        </Link>
      </Panel>
    );
  ...
  return (
    <>
      ...
      {tab === "Documents" ? (
        <Panel title="Documents" action={<Link to="/documents" className="text-xs font-semibold text-colorLink">Document register</Link>}>
