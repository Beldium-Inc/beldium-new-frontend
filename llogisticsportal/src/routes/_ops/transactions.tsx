...
export const Route = createFileRoute("/_ops/transactions")({
  validateSearch: tabSearch,
  head: () => ({
    meta: [
      { title: "Transactions | Beldium Logistics" },
      { name: "description", content: "Every RFQ and transaction with buyer, miner, mine, batch, quantity, origin and destination." },
      { property: "og:title", content: "Transactions | Beldium Logistics" },
    ...
    <>
      ...
      <Panel title="Transaction Register">
        <QueueView
          ...
          onOpen={(t) => navigate({ to: "/transactions/$txnId", params: { txnId: t.id } })}
        />
      </Panel>
    </>
  );
