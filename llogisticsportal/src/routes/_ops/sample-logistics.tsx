...
export const Route = createFileRoute("/_ops/sample-logistics")({
  validateSearch: tabSearch,
  head: () => ({
    meta: [
      { title: "Sample Logistics | Beldium Logistics" },
      { name: "description", content: "Sample pickup queue with chain of custody from mine to laboratory receipt." },
      { property: "og:title", content: "Sample Logistics | Beldium Logistics" },
    ...
    <>
      ...
      <Panel title="Sample Queue">
        <QueueView
          ...
          onOpen={(r) => (r.req ? navigate({ to: "/transport-requests/$requestId", params: { requestId: r.req.id } }) : navigate({ to: "/movements/$movementId", params: { movementId: r.mov!.id } }))}
          actions={(r) =>
            r.req ? (
              <RowAction
                onClick={() => {
