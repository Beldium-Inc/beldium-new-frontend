...
export const Route = createFileRoute("/_ops/drivers")({
  validateSearch: tabSearch,
  head: () => ({
    meta: [
      { title: "Drivers | Beldium Logistics" },
      { name: "description", content: "Driver queues: available, assigned, on journey, off duty and compliance hold." },
      { property: "og:title", content: "Drivers | Beldium Logistics" },
    ...
    <>
      ...
      <Panel title="Driver Queue">
        <QueueView
          ...
          onOpen={(d) => navigate({ to: "/drivers/$driverId", params: { driverId: d.id } })}
        />
      </Panel>
      <Modal
        open={add}
