...
export const Route = createFileRoute("/portal/vehicles_/$vehicleId")({
  head: ({ params }) => ({
    meta: [
      { title: `${params.vehicleId} | Vehicle - Beldium Logistics Hub` },
      { name: "description", content: `Vehicle ${params.vehicleId} profile: assignment, documents, compliance, maintenance and history.` },
      { property: "og:title", content: `${params.vehicleId} - Beldium Logistics Hub` },
...
function VehicleProfile() {
  ...
  if (!v) {
    return (
      <Panel title="Vehicle not found">
        <Link to="/portal/fleet" className="text-sm font-semibold text-colorLink">
          Back to Fleet
        </Link>
      </Panel>
    );
  ...
  return (
    <>
      ...
      {tab === "Documents" ? (
        <Panel title="Documents" action={<Link to="/portal/documents" search={{ tab: "All" }} className="text-xs font-semibold text-colorLink">Document register</Link>}>
