...
import { Shell, PageHeader } from "@/components/beldium/shell";
import { ApplicationStatusPanel } from "@/components/beldium/application-status";
...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "Logistics Command Centre | Beldium" },
      ...
      { property: "og:title", content: "Logistics Command Centre | Beldium" },
...
function Dashboard() {
  ...
  const kpis: { label: string; value: number; tone: string; target: Target }[] = [
    { label: "New requests", value: newReqs.length, tone: "text-colorLink", target: { kind: "queue", path: "/transport-requests", tab: "Awaiting Decision" } },
    { label: "Active movements", value: active.filter((m) => m.stage !== "Awaiting Assignment").length, tone: "text-primary", target: { kind: "queue", path: "/active-movements" } },
    { label: "In transit", value: active.filter((m) => m.stage === "In Transit").length, tone: "text-colorLink", target: { kind: "queue", path: "/active-movements", tab: "In Transit" } },
    { label: "Exceptions", value: exceptions.length, tone: "text-destructive", target: { kind: "queue", path: "/active-movements", tab: "Exceptions" } },
    { label: "Delivered today", value: s.movements.filter((m) => m.completedAt && new Date(m.completedAt).toDateString() === today).length, tone: "text-success", target: { kind: "queue", path: "/deliveries", tab: "Completed" } },
    { label: "Available vehicles", value: s.vehicles.filter((v) => vehicleAvailability(v, s) === "Available").length, tone: "text-success", target: { kind: "queue", path: "/fleet", tab: "Available" } },
    { label: "Available drivers", value: s.drivers.filter((d) => driverAvailability(d, s) === "Available").length, tone: "text-success", target: { kind: "queue", path: "/drivers", tab: "Available" } },
    { label: "Open incidents", value: s.incidents.filter((i) => i.status === "Open").length, tone: "text-warning", target: { kind: "queue", path: "/incidents", tab: "Open" } },
  ];
  ...
  return (
    <Shell>
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-2">
        <Panel title="New Requests" action={<Link to="/transport-requests" search={{ tab: "Awaiting Decision" }} className="text-xs font-semibold text-colorLink">Open queue</Link>}>
        ...
        <Panel title="Active Movements" action={<Link to="/active-movements" className="text-xs font-semibold text-colorLink">Open queue</Link>}>
          <ul className="divide-y divide-border">
        ...
        <Panel title="Exceptions" action={<Link to="/active-movements" search={{ tab: "Exceptions" }} className="text-xs font-semibold text-colorLink">Open queue</Link>}>
          <ul className="divide-y divide-border">
        ...
        <Panel title="Notifications" action={<Link to="/notifications" className="text-xs font-semibold text-colorLink">View all</Link>}>
          <ul className="divide-y divide-border">
            {notes.map((n) => (
              <li key={n.id}>
                <button
