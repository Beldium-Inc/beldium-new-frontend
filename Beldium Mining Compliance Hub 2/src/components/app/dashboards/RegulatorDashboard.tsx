...
export function RegulatorDashboard() {
  ...
  const { verification, partnerId, approvedCapabilities, regulator } = useOnboarding();
  const verifiedOrg = verification === "Verified" && partnerId?.startsWith("BLD-RO");
  ...
  return (
    <>
      ...
      />
      {verifiedOrg && (
        <div className="mb-5 rounded-xl border border-success/40 bg-success/15 p-4">
          <p className="text-sm font-semibold">
            {regulator.legalName || "Your organisation"} is VERIFIED · Beldium Regulatory Organisation ID {partnerId}
          </p>
          <p className="mt-1 text-xs text-muted-foreground">
            Activated oversight capabilities: {approvedCapabilities.join(", ") || "None recorded"}
          </p>
        </div>
      )}
